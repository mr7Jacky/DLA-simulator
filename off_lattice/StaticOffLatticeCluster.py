...
class StaticOffLatticeCluster:
    ...
    def __init__(self, particle_radius=1, lattice_size=500, delta=2 * np.pi, max_num_particle=3000):
    ...
    def near_cluster(self, x, y):
        ...
        for particle in self.cluster:
            if hp.cal_two_points_dist(particle.loc, (x, y)) <= self.particle_radius:
                return True
        return False
    ...
    def generate_random_point(self):
        ...
        x = self.mid + ((self.r_max + 2) * np.cos(theta_rand))
        y = self.mid + ((self.r_max + 2) * np.sin(theta_rand))
        return x, y
